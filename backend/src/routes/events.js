...
async function categoryExists(householdId, name) {
  return Category.exists({ household: householdId, scope: 'event', name });
}
...
router.put('/reorder', async (req, res) => {
  ...
  await Promise.all(ids.map((id, index) => Event.updateOne({ _id: id, household: req.householdId }, { order: index })));
  res.json({ ok: true });
});

router.get('/:id', async (req, res) => {
  const event = await Event.findOne({ _id: req.params.id, household: req.householdId }).populate('owner', 'name');
...
router.post('/', async (req, res) => {
  ...
  if (!(await categoryExists(req.householdId, type))) {
  ...
  if (!startTime) {
    ...
    const last = await Event.findOne({ household: req.householdId, date: { $gte: eventDate, $lt: dayEnd }, startTime: null })
      .sort({ order: -1 })
      .select('order');
...
router.put('/:id', async (req, res) => {
  const { title, type, notes, date, startTime, reminderEnabled, reminderAt } = req.body;
  const event = await Event.findOne({ _id: req.params.id, household: req.householdId });
  if (!event) return res.status(404).json({ error: 'Event not found' });
